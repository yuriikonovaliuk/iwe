...
fn apply_changes_remove_begins_and_commits_on_the_stub() {
    ...
    apply_changes_with(&changes, dir.path(), Format::Markdown, |_, _, _, _| Ok(()), None, {
        let log = log.clone();
...
fn always_rejecting_check_hook_aborts_and_leaves_no_partial_state() {
    ...
    let result = apply_changes_with(&changes, dir.path(), Format::Markdown, always_rejecting, None, {
        let log = log.clone();
        move || RecordingTransaction::new(log.clone())
    });
