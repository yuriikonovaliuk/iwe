...
The `[integrity]` section: structural link integrity, refused at the commit every write goes through
...
- A **broken link** is an internal link or inclusion to a key that has no
  document. External URLs are not internal links; a `#fragment` is dropped
